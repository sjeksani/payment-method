...
/**
 * AccountEditViewUse class is responsible to read the authored content for the Account Edit component.
 * 
 * @author Jai Parkash
 *
 */

public class AccountEditViewUse extends WCMUsePojo {
  private static final Logger LOGGER = LoggerFactory.getLogger(AccountEditViewUse.class);
  ...
  public void activate() throws Exception {
	    ...
	    try{
	    	...
	    	// parent(parsys)/parent(modal)/parent(page)/paymentmethod
	    	//Resource resource = getResourceResolver().getResource("../../../paymentmethod");
	    	Resource resource = getResource().getParent().getParent().getParent().getChild("paymentmethod");
	...
	//	        modalNameOnAcctLabel = props.get(MODAL_NAMEONACCOUNT_LBL, "");
		    	...
		    	LOGGER.error("Unable to get a reference to Payment Method component.");
	    		
	    	}

	        
	    } catch (Exception e){
	    	LOGGER.error("Exception while loading AccountEditViewUse:" + e.getMessage());
	    }
		if (debugEnabled)
  ...
  public String getFNameLabel() {
    ...
    /**
     * This method is responsible to get the edit modal text for ACH.
     * 
     * @return String
     */
    public String getACHModalText() {
      return achModalText;
    }
